...
t_qtree             *get_next_gen(t_qtree *qtree, t_env *env)
{
    ...
    if (qtree->next)
        return (qtree->next);
    if (qtree->level == 2)
        return (apply_rules(qtree, env));//TODO hash here or in aplyy rules func
    n00 = get_next_gen(qtree->nw, env);
    n01 = r_on_horizon(qtree->nw, qtree->ne, env);//TODO skip one level (2)
    ...
    return (new_node(qtmp1, qtmp2, qtmp3, qtmp4));
}